module Assembler (headerFile) where
...
-- Converte o código em assembly para um .mif
convertCode :: String -> IO ()
convertCode fileName = do
    curDir <- getCurrentDirectory
    contents <- readFile (curDir ++ "/tmp/" ++ fileName)
    writeFile (curDir ++ "/tmp/instructions.mif") ( unlines (headerFile ( lines contents ) ( -1 ) ( length ( lines contents ) ) ) )
